...
\begin{enumerate}
    ...
    \item \textbf{Aplicación del Modelo FMM:} Una vez establecido el orden circular preliminar de las muestras, CIRCUST modela las curvas de expresión génica. Históricamente, este paso se ha abordado mediante modelos armónicos clásicos como el \textbf{Cosinor} \cite{cosinor}. Sin embargo, el Cosinor resulta demasiado rígido para capturar los patrones de expresión asimétricos típicos del transcriptoma, y su extensión multicomponente tiende a generar un grave sobreajuste (\textit{overfitting}). Por otro lado, enfoques no paramétricos como \textbf{ORI} (\textit{Order Restricted Inference}) \cite{larribaORI} ofrecen flexibilidad al modelar oscilaciones mediante restricciones de desigualdad, pero carecen de parámetros matemáticos directos que faciliten la comparación analítica entre distintos ritmos.
    Para superar ambas limitaciones, CIRCUST emplea el modelo \textbf{FMM} (\textit{Frequency Modulated Möbius}) \cite{fmmAlgorithm}. Este modelo paramétrico no lineal de cinco parámetros permite deformar la onda cosenoidal clásica para ajustarse a expresiones asimétricas y complejas. Con ello se logra una estimación robusta, biológicamente realista y cuyos parámetros conservan una interpretación directa sobre la fase oscilatoria del reloj biológico.
\end{enumerate}
...
\bibitem{larribaORI}
Larriba, Y., Rueda, C., Fernández, M. A., y Peddada, S. (2020). 
