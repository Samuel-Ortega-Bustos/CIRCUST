...
\subsection{Alternativas Computacionales: CYCLOPS, Aprendizaje Profundo, PENN y CHIRAL}
...
En particular, la incorporación de mecanismos de corrección de covariables mediante regresión lineal permitiría integrar la interpretabilidad propia de la estadística clásica con la robustez necesaria para abordar conjuntos de datos biológicos heterogéneos, acercando el rendimiento de estos enfoques al de los modelos de aprendizaje profundo.
\section{Limitaciones Existentes y Motivación del Trabajo}
A pesar de la validez teórica de métodos basados en CPCA como CIRCUST, su aplicación práctica en estudios transcriptómicos a gran escala se ve obstaculizada por importantes limitaciones de software y estadístico-matemáticas que este Trabajo de Fin de Grado pretende resolver:

\begin{enumerate}
    \item \textbf{Falta de Modularidad y Extensibilidad:} La implementación actual de muchos de estos algoritmos (frecuentemente desarrollados como scripts monolíticos en R) carece de una arquitectura de software moderna. Esto dificulta enormemente la integración del algoritmo en pipelines de bioinformática más amplios, la reutilización de código y la experimentación eficiente.

    \item \textbf{Susceptibilidad a Covariables (Confounding Factors):} En estudios humanos reales, la expresión génica está altamente influenciada por variables no circadianas (covariables), como la edad, el sexo, el estado patológico o efectos técnicos de lote (batch effects). Actualmente, CIRCUST no dispone de un marco estadístico integrado para ajustar los Componentes Principales Circulares (CPCs) respecto a estas covariables, lo que puede introducir sesgos severos en la estimación de la fase.

    \item \textbf{Muestreo Disperso (Sparse Sampling):} Los algoritmos actuales sufren pérdidas de precisión cuando la distribución de las muestras a lo largo del ciclo de 24 horas no es uniforme. Bajo condiciones de escasez de datos en ciertas franjas horarias, la inferencia del orden circular se vuelve inestable.
\end{enumerate}

En respuesta a estas problemáticas, este proyecto se centra en la re-implementación algorítmica y el refinamiento estadístico de los métodos de ordenación circular, con el fin de proporcionar una herramienta computacional robusta, modular y analíticamente superior para entornos de datos ruidosos y con factores de confusión.
\chapter{Objetivos}
...
\begin{enumerate}
    ...
    \item \textbf{Desarrollo de un Marco de Simulación:} Diseñar un entorno de simulación capaz de generar conjuntos de datos transcriptómicos sintéticos. Esto permitirá evaluar sistemáticamente la robustez del nuevo algoritmo bajo condiciones controladas de perturbación, tales como distintos niveles de ruido, escasez de muestras y efectos por lotes.
    \item \textbf{Validación con Datos Humanos Reales:} Aplicar el pipeline mejorado a conjuntos de datos transcriptómicos reales provenientes de tejido cerebral humano post-mortem (con metadatos de hora de fallecimiento conocidos) para cuantificar la precisión de la inferencia de fase y su relevancia biológica.
\end{enumerate}
